...
`/admin/ghost` is the owner's ghost overlay: a Deltarune Ralsei sprite that can be driven around a visitor's screen, speak in a typewriter bubble, hold a pose (stand, shy, wave, hide, sing, bow, aura, asleep in bed, hooded, hat off, under the hat, squashed, knocked out, heap), and run one-shot actions (heal, pacify, scarf whip, dark world, and friends). It is page-scoped, and the audience is one of everyone on that page, one signed-in viewer picked off the presence roster, or nobody (staging, where only the owner sees him); the owner always sees his own ghost. The control stage frames the real page in an iframe at the watching viewer's own viewport size, scaled to fit, so aiming next to someone's avatar lands on the same pixels for them; the framed copy asks the overlay to stay asleep through a `#ghost-preview` hash so it neither joins the roster nor draws a second Ralsei. Backend state lives in `live-backend/src/live/ghost.ts` (`GhostHub`, in-memory, serving process only, nothing durable, gated by `ENABLE_GHOST`); the sprite atlas is `public/images/ghost/ralsei.png` (packed from the ripped Chapter 1+2 and Chapter 5 sheets) with its clip manifest in `src/lib/ghost-shared.ts`; the URL carries `GHOST_ATLAS_VERSION` because the service worker caches `/images/*` forever, and a test pins the file's hash so changing the art without bumping it fails, and the visitor overlay is `src/components/ghost/` mounted once from `__root.tsx`.
...
`pack-duels.ts` is the head-to-head layer, and is **admin-gated while it is prototyped**: the frontend server fns in `src/lib/pack-duels.ts` all call `requireAdminAccess`, and the `/duel/{id}` page plus both entry points on `/packs` are `canUseAdminFeatures`-gated. `pack_duels` holds one row per duel, of two kinds. A **challenge** freezes the hand from a pack the challenger opened; the opponent opens their own pack of the same type on the duel page, and the duel resolves on submission (higher total card power, tiebroken on the single best card). A **blackjack** duel deals a 14-card deck at creation, gives each side its own half of it, and each plays against a target of 21: two cards to start, then hit or stand, bust over the target. A card is worth its star rating (`value`, the number already printed on the card front). Both sides play at the same time and blind, so neither waits for a turn and neither gets the dealer's advantage of acting last; closest to 21 without busting wins, both bust or level is a tie. Nothing here touches the economy: duel cards never enter a collection, mint a serial or move a shard, and card values are client-reported like the pull log. The parts that would decide a game are server-held: the deck is frozen at creation, the server deals from it, and `redactDuelFor` hides every hand still in play from anyone but its owner (an open challenge stays sealed from everyone, so it cannot be sized up before being accepted). Endpoints: `POST /api/packs/duels` (create), `POST /api/packs/duels/{id}/{join,hit,stand,view}` (all admin-token gated and viewer-forwarded; `view` is the signed-in read that can show you your own hand), and public `GET /api/packs/duels/{id}` (every live hand hidden). Dev-only self-duel hatch: the backend accepts a join from the challenger when `nodeEnv !== "production"` (and then applies a move to whichever of their two hands is still live), and the duel page shows the seat button to the challenger under `import.meta.env.DEV`, so one osu! login can play a whole duel locally. Both gates are independent, and neither exists in a production build.
