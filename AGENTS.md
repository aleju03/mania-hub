...
- `npm run dan:benchmark` runs the dan classifier against curated benchmark beatmapsets and compares predicted dan against expected labels from Turso, using the same dataset as the admin `/admin/dan-classifier` benchmark tab. Useful flags include `--family normal|ln`, `--classifier aleju|daniel`, `--rate N`, `--set IDS`, `--beatmap IDS`, `--expected LABELS`/`--label LABELS`, `--match exact|base|wrong|unlabeled`, `--grep TEXT`, `--failures-from FILE`, `--show-changes FILE`, `--compare BEFORE AFTER`, `--summary-by-set`, `--explain-wrong`, `--debug`, `--json`, and `--include-unlabeled`. Use `--set`/`--beatmap` for focused iteration, `--grep` to inspect a diff or pack by title/version text, `--json > before.json` followed by `--failures-from before.json --match wrong --explain-wrong` to rerun current misses with calibration diagnostics, `--show-changes before.json` to report current prediction deltas after a new run, and `--compare before.json after.json --summary-by-set` to compare two existing JSON outputs without rerunning the classifier. Use `npm run --silent dan:benchmark -- --json` for machine-readable output so npm header lines do not pollute stdout. Requires `TURSO_DATABASE_URL` and `TURSO_AUTH_TOKEN` except for comparing existing JSON files with `--compare`.
