...
- Endpoints: `GET /api/ghost/stream?route=` (public SSE, one per page open, own budget via `GHOST_MAX_CLIENTS` / `GHOST_MAX_CLIENTS_PER_IP`, never the live-feed one; also the presence roster), `POST /api/ghost/ticket` (admin-token gated, `getGhostControlTicket` in `src/lib/ghost.ts`), `POST /api/ghost/control?ticket=` (driven from the browser at ~15 Hz so movement never pays for a hop through the frontend server), `GET /api/ghost/presence?ticket=` (per-route counts capped at 40 routes, identities for signed-in viewers capped at 120), `POST /api/ghost/say` (viewer replies, connection id from their own `hello` event, 6 per 30s, control characters stripped so a reply cannot forge an SSE frame), `GET /api/ghost/inbox?ticket=` (replies, 40-entry in-memory ring, never SQLite).
