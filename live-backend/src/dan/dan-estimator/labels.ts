import type { DanEstimateInput, DanPrimaryFamily } from "./types.js";
...
export function srToRawDan(sr: number, family: DanPrimaryFamily, options: { calibrate?: boolean } = {}): number {
  const calibratedSr = options.calibrate === false ? sr : calibrateSrForFamily(sr, family);
  return rawDanFromMeans(calibratedSr, DAN_MEANS[family]);
}
