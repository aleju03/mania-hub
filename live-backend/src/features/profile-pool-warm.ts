...
/* Pool players (tracked roster members, the same set pack draws come from)
   with no stored best scores anywhere. pp desc so the top slices (elite and
   legend packs) go warm first. */
    ...
    `select u.user_id
     ...
     where u.pp is not null
       ...
       and exists (
         ...
         where ro.user_id = u.user_id and ro.is_tracked = 1 and ro.rank is not null
       )
