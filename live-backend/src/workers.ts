...
import { BRACKET_CONTENT_RECOMPUTE_JOB, BRACKET_TAG_RECOMPUTE_JOB, CHART_ANALYSIS_BACKFILL_JOB, CHART_ANALYSIS_JOB, CHORDJACK_TAG_RECOMPUTE_JOB, COMPANELLA_RECOMPUTE_JOB, DAN_ELIGIBILITY_RECOMPUTE_JOB, DAN_FLOOR_PIN_RECOMPUTE_JOB, DT_RATE_ANALYSIS_JOB, HT_RATE_ANALYSIS_JOB, INVERSE_CLUSTER_BPM_JOB, JACK_DEMAND_RECOMPUTE_JOB, NKEY_MSD_JOB, JACK_TAG_RECOMPUTE_JOB, MOTION_FEATURES_RECOMPUTE_JOB, LEOBLACK_REPIN_DT_RECOMPUTE_JOB, LEOBLACK_REPIN_RECOMPUTE_JOB, LN_MSD_SWEEP_JOB, LN_EFFECTIVE_RECOMPUTE_JOB, LN_LEOBLACK_RECOMPUTE_JOB, LN_PRIMARY_REPIN_JOB, LN7_PRIMARY_REPIN_JOB, LN_SOURCE_RECOMPUTE_JOB, LN_SUBTYPE_RECOMPUTE_JOB, MSD_POISON_RECOVERY_JOB, NOTE_BPM_RECOMPUTE_JOB, OSU_FILE_REPAIR_JOB, SUNNY_REPIN_DT_RECOMPUTE_JOB, SUNNY_REPIN_RECOMPUTE_JOB, VIBRO_RECOMPUTE_JOB, computeBeatmapChartAnalysis, runBracketContentRecomputeJob, runBracketTagRecomputeJob, runChartAnalysisBackfillJob, runChordjackTagRecomputeJob, runCompanellaRecomputeJob, runDanEligibilityRecomputeJob, runDanFloorPinRecomputeJob, runDtRateAnalysisJob, runHtRateAnalysisJob, runInverseClusterBpmRecoveryJob, runJackDemandRecomputeJob, runNkeyMsdJob, runJackTagRecomputeJob, runMotionFeaturesRecomputeJob, runLeoblackRepinDtRecomputeJob, runLeoblackRepinRecomputeJob, runLnLeoblackRecomputeJob, runLnMsdSweepJob, runLnEffectiveRecomputeJob, runLn7PrimaryRepinJob, runLnPrimaryRepinJob, runLnSourceRecomputeJob, runLnSubtypeRecomputeJob, runMsdPoisonRecoveryJob, runNoteBpmRecomputeJob, runOsuFileRepairJob, runSunnyRepinDtRecomputeJob, runSunnyRepinRecomputeJob, runVibroRecomputeJob } from "./features/chart-analysis.js";
...
const DEFAULT_WORKER_LANES: WorkerLane[] = [
  ...
  },
  {
    ...
    // lane starved: the chart-family/credential sweep (~234k files) and the
    // stored-dan refold (~24k players). In the chart lane their continuation
    // re-queued behind every changed-file repair page and won one claim in
    // ~500 (one 50-row chunk per ten minutes on prod). Here they share one
    // explicit budget: one chunk, then this gap, alternating with each other
    // through equal priority. No osu! API, no MinaCalc; the gap is what
    // keeps the worker's event loop free for the other lanes.
    name: "maintenance-sweeps",
    jobTypes: [CHART_FAMILY_SWEEP_JOB, PLAYER_SKILL_DAN_SWEEP_JOB],
    claimLimit: 1,
    intervalMs: readConfig().maintenanceSweepLaneIntervalMs,
  },
  {
    name: "osu-file-cache",
...
export class WorkerRunner {
  ...
  private async handle(job: Job, signal?: AbortSignal): Promise<void> {
    ...
    if (job.type === BEATMAP_FILE_CHANGE_JOB) {
      await runChangedBeatmapFileRepairJob(this.db, this.queue, job.payload as ChangedBeatmapFileRepairPayload);
      return;
    }
    ...
    if (job.type === COMPANELLA_INPUTS_JOB) {
      await runCompanellaInputsJob(this.db, this.queue, job.payload as { cursor?: number });
      return;
    }
    if (job.type === MARATHON_CORRECTION_JOB) {
      await runMarathonCorrectionJob(this.db, this.queue, job.payload as { cursor?: number });
      return;
    }
    ...
    if (job.type === LN_EFFECTIVE_RECOMPUTE_JOB) {
      if (await runLnEffectiveRecomputeJob(this.db, this.queue, job.payload as { cursor?: number })) {
        await ensurePlayerSkillDanSweepSeeded(this.db, this.queue);
        await ensurePlayerSkillPatternSweepSeeded(this.db, this.queue);
      }
      return;
