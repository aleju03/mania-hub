// Shared by the live history recorder and every frontend Maniacard surface.
// Keep one trait model so recorded ratings and card renders cannot diverge.
import type { OscScore, OsuBeatmap, OsuScoreStatistics } from "./types.js";
import { calculateWeightedPpTotal, getModAcronyms, getScoreRate, getStableScaleManiaAccuracy, isLazerScore } from "./score.js";

export type ManiaCardScore = Omit<OscScore, "beatmap"> & {
  beatmap: OsuBeatmap & {
    accuracy: number;
    drain: number;
    total_length: number;
...
function isUsable(score: ManiaCardScore): boolean {
...
function getKeyMode(score: ManiaCardScore): number {
...
function scoreWeight(score: ManiaCardScore, index: number, keyModeCount: number): number {
...
function getEffectiveStarRating(score: ManiaCardScore, rate: number): number {
...
function computePlayTraits(score: ManiaCardScore, baseline: KeymodeBaseline) {
  const b = score.beatmap;
...
export function resolveCardGlobalPp(
  ...
  scores: Array<Pick<ManiaCardScore, "pp" | "id">>,
): number {
...
export function computeManiaSkills(scores: ManiaCardScore[], options: { globalPp?: number | null } = {}): ManiaSkills | null {
