...
describe("recomputePlayerSkillDanChunk", () => {
  ...
  it("uses the full scope when only the pre-7K-LN v21 marker exists", async () => {
    const db = await makeDb();
    const queue = new JobQueue(db);
    ...
    await exec(
      ...
      ["player_skill_dan_sweep_done:v21", json({ finishedAt: "2026-08-30T00:00:00.000Z" }), "2026-08-30T00:00:00.000Z"],
    );
    await ensurePlayerSkillDanSweepSeeded(db, queue);
