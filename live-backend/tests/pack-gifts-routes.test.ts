...
const { bridge, body, send, transfer, inbox, ack, search } = vi.hoisted(() => ({ bridge: vi.fn(), body: vi.fn(), send: vi.fn(), transfer: vi.fn(), inbox: vi.fn(), ack: vi.fn(), search: vi.fn() }));
...
vi.mock("../src/features/pack-gifts.js", () => ({ sendPackGift: transfer, listPackGiftInbox: inbox, acknowledgePackGifts: ack, searchGiftCollectors: search }));
...
it("takes the sender from the verified path, never a sender id in the body", async () => {
  ...
  expect(transfer).toHaveBeenCalledWith(ctx.db, 101, { recipientUserId: 102, cardKey: "77", requestId: "gift-request-00000001" });
});
it("reads and dismisses only the bridge-supplied recipient's inbox", async () => {
  ...
  expect(inbox).toHaveBeenCalledWith(ctx.db, 101);
