...
import { replayOverlayCenteredY, replayOverlayLayoutScale, replayOverlayScale, replayOverlayX } from "./replay-overlay-layout";
...
describe("replay overlay layout", () => {
  it.each([0.35, 0.5, 0.65])("keeps the leaderboard center at %s when fullscreen shrinks its side group", (center) => {
    ...
    const y = center - originalHeight / (2 * source.height);
    const height = originalHeight * replayOverlayLayoutScale(source, fullscreen);
    expect(height).toBeLessThan(originalHeight);
    expect((replayOverlayCenteredY(y, height, source, fullscreen) + height / 2) / fullscreen.h).toBeCloseTo(center);
    expect(replayOverlayCenteredY(y, originalHeight, source, inline)).toBeCloseTo(y * inline.h);
  });
  ...
  it("keeps right-side overlays outside the playfield and preserves their clearance", () => {
    ...
    expect(x - right).toBeGreaterThanOrEqual(0.76 * reference.width - oldRight);
  });
  ...
  it("keeps a complete left-side overlay outside the playfield", () => {
    ...
    expect(x).toBeCloseTo(0.15 * reference.width);
  });
  ...
  it("keeps export resolution independent of overlay placement and scale", () => {
    for (const height of [720, 1080, 1440]) {
      ...
      expect(replayOverlayScale(reference, stage) / ratio).toBeCloseTo(replayOverlayScale(reference, fullscreen));
    }
  });
});
