...
describe("rank page math", () => {
  it("maps ranks to rankings pages and row indices", () => {
    ...
    expect(rankToPage(10_000)).toBe(200);
  });
});
...
describe("pickPackEntries", () => {
  ...
  it("skips players already claimed earlier in the pack", () => {
...
describe("tracked pool draws", () => {
  ...
  it("fails the deal on a pool failure instead of spending osu! API calls", async () => {
    // The fallback draw this used to degrade to is deliberately gone: on
    // 2026-08-07 spammed opens got their pool reads 429ed and every one of
    // them turned into real osu! rankings fetches. Any pool failure now
    // surfaces as a failed deal.
    vi.mocked(fetchLiveGlobalRankings).mockRejectedValue(new Error("backend down"));
    vi.mocked(getRankings).mockClear();
    await expect(drawPackPlayers(mulberry32(5), { count: 12 })).rejects.toThrow();
    expect(getRankings).not.toHaveBeenCalled();
  });

  it("retries once when the pool read is rate-limited and about to clear", async () => {
    const { fetchPage } = makePoolFetcher(5853);
    vi.mocked(fetchLiveGlobalRankings)
      .mockRejectedValueOnce(new LiveBackendRequestError(429, 50))
      .mockImplementation(async (options) => {
        const params = typeof options === "number" || options == null ? { page: 1 } : options;
        const page = await fetchPage(params.page ?? 1);
        return { ...page, page: params.page ?? 1, pageSize: RANKINGS_PAGE_SIZE, fetchedAt: Date.now() };
      });
    const { players } = await drawPackPlayers(mulberry32(21));
    expect(players).toHaveLength(PACK_SIZE);
  });

  it("fails fast on a rate limit that asks for a long wait", async () => {
    // Stalling a reveal for the window reset is worse than showing the retry
    // button, and hammering the read again is what the 429 asked us not to do.
    vi.mocked(fetchLiveGlobalRankings).mockReset();
    vi.mocked(fetchLiveGlobalRankings).mockRejectedValue(new LiveBackendRequestError(429, 30_000));
    vi.mocked(getRankings).mockClear();
    await expect(drawPackPlayers(mulberry32(5))).rejects.toThrow();
    expect(vi.mocked(fetchLiveGlobalRankings).mock.calls.length).toBe(1);
    expect(getRankings).not.toHaveBeenCalled();
  });
...
// What toPackPlayer did for the deleted osu!-rankings draw, kept local to the
// tests that build hands straight from ranking-shaped entries.
function entryToPackPlayer(entry: LeanRankingEntry): PackPlayer {
  return {
    user: {
      id: entry.user.id,
...
describe("the honorary slot", () => {
  const pack = (size: number) => Array.from({ length: size }, (_, index) => makeEntry(9000 - index * 100)).map(entryToPackPlayer);
...
describe("reveal order", () => {
  it("sorts weakest rank first so the hit comes last", () => {
    const players = [makeEntry(40), makeEntry(9000), makeEntry(700)].map(entryToPackPlayer);
