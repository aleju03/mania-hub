...
import { useCommunityErrorMessage, useCommunityReportReasonLabel } from "./field-options";
...
export function CommunityReportModal({
  ...
  const reasonLabel = useCommunityReportReasonLabel();
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={t`Report ${community.name}`}
      onClick={(event) => {
    ...
    >
      <div className="my-auto w-full max-w-md rounded-xl border border-osu-b3/30 bg-osu-b5">
        <div className="flex items-center gap-2.5 border-b border-osu-b3/30 px-4 py-3">
          <h2 className="min-w-0 flex-1 truncate text-[14px] font-bold text-white"><Trans>Report {community.name}</Trans></h2>
          <button
            type="button"
            onClick={onClose}
            aria-label={t`Close`}
