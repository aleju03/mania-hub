...
// False during SSR and the hydration render, true from the post-mount render on. React skips
// attribute diffing while hydrating, so a style that exists only in the client render (persisted
// accents seed the store before hydration) is silently never written to the DOM - and since the
// store value doesn't change afterwards, nothing re-renders and the name stays uncolored until
// another visit misses the cache. Deferring store accents to the post-hydration render makes the
// color land through a normal style-prop diff instead.
