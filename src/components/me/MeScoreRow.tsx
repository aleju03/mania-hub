...
function keymodeLabel(score: LeanTrackerScore): string | null {
...
function coverThumb(score: LeanTrackerScore): string | null {
...
export function MeScoreRow({ score, isNew, ppGain }: { score: LeanTrackerScore; isNew?: boolean; ppGain?: number }) {
    ...
    <div className={`flex items-center gap-2.5 rounded-lg border border-osu-b3/20 bg-osu-b4 py-2 pl-2 pr-2.5 transition-colors hover:bg-osu-b3/40${isNew ? " score-enter" : ""}`}>
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 flex min-h-[13px] flex-wrap items-center gap-0.5">
          {mods.length > 0 ? mods.map((m) => <ModBadge key={m.acronym} mod={m.acronym} rate={m.rate} size={0.55} />) : <span className="text-[9px] text-osu-f1/70">nomod</span>}
        </div>
      </div>
      <div className="flex shrink-0 flex-col items-end">
        <div className="flex items-center gap-2">
          <span className={`text-[11px] tabular-nums ${accColor(acc)}`}>{formatAccuracy(acc)}</span>
          <span className="text-[13px] font-bold tabular-nums text-white">{formatPP(score.pp)}</span>
