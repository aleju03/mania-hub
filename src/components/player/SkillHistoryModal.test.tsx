...
vi.mock("../../lib/player-skill-history", () => ({ fetchPlayerSkillHistory: fetchHistory }));
...
describe("skill history", () => {
  it("shows compact changes and only reveals skill details when a row is opened", async () => {
    ...
    expect(fetchHistory).toHaveBeenCalledWith({ data: { userId: 99, keyCount: 7, before: undefined }, signal: expect.any(AbortSignal) });
  ...
  it("retains the loaded history when an older page fails and retries that cursor", async () => {
    ...
    expect(fetchHistory).toHaveBeenLastCalledWith({ data: { userId: 99, keyCount: 7, before: 2 }, signal: expect.any(AbortSignal) });
  ...
  it("previews realistic gains and drops locally and restores the real history", async () => {
    ...
    expect(view.getAllByRole("listitem")).toHaveLength(13);
  ...
  it("hides history from non-admins, including viewers with only dev-feature access", () => {
    authFlags.canUseAdminFeatures = false;
    authFlags.canUseDevFeatures = true;
    const view = render(<I18nProvider i18n={getI18n("en")}><SkillHistoryButton userId={99} keyCount={7} /></I18nProvider>);
  ...
  it("closes an open history window when admin access is lost", async () => {
    fetchHistory.mockResolvedValue({ items: [], nextBefore: null });
